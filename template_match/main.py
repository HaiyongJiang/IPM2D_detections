...
def match_template(fpath_mainimage, fpath_template, th=0.3):
    """
    params:
    ...
    """
    print("Processing ", fpath_mainimage)
	# load data
    ...
    cands = []
    for rot in np.linspace(0, 180, 60):
        center = (w/2, h/2)
        ...
        for scale in np.linspace(0.2, 2.0, 25)[::-1]:
            resized = cv2.resize(
                img_gray,
    ...
    print("Filtered results: ", max_loc.transpose(1,0))


    #  (startX, startY) = max_loc[0], max_loc[1]
    #  (endX, endY) = max_loc[0] + (w * s_r).astype(np.int32), max_loc[1] + (h * s_r).astype(np.int32)
    # draw a bounding box around the detected result and display the image
    #  cv2.rectangle(img_rgb, (startX, startY), (endX, endY), (0, 0, 255), 5)
    #  cv2.rectangle(img_rgb, max_loc[0], max_loc[0], (0, 0, 255), 5)
    #  for ii in range(len(startX)):
    #      cv2.rectangle(img_rgb, (startX[ii], startY[ii]),
    #                    (endX[ii], endY[ii]), (0, 0, 255), 5)
