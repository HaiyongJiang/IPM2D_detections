/**
 * File              : samples/cpp/generalized_hough.cpp
...
int main(int argc, const char* argv[])
{
    CommandLineParser cmd(argc, argv,
        ...
        "{ votesThreshold | 300        | the accumulator threshold for the template centers at the detection stage. The smaller it is, the more false positions may be detected }"
        "{ angleThresh    | 100     | angle votes threshold }"
        "{ scaleThresh    | 100      | scale votes threshold }"
    ...
    cout << "Detected instances: " << position.size() << "\n";
    ...
    std::ofstream fid;
    fid.open( (imageName + string(".txt")).c_str() );
